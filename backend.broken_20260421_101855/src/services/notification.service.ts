...
const reminderTemplates = [
  ...
  // Тип 2: Вопросный
  (name: string, goalTitle: string, streakText: string) => 
    `${name}, что было самым ценным сегодня в работе над цели?${streakText}\n\n` +
    `📌 Цель: <b>${goalTitle}</b>\n\n` +
...
/**
 * Обновляем функцию scheduleDailyReminder для использования разнообразных шаблонов
 */
  ...
  cron.schedule('0 * * * *', async () => {
    ...
    try {
      ...
      for (const user of users) {
        ...
        const templateIndex = Math.floor(Math.random() * reminderTemplates.length);
        const message = reminderTemplates[templateIndex](name, goal.title, streakText, stats);
